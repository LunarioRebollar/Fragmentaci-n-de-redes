...
var addressBlock = 'Ingresa direccion IP'; //No I18N
...
function getSubnetDetails(resultArray){
	...
	for(var j=0; j<resultArray.length; j++){
		...
		startIpAddress = startAddressArr.join('.');
		endIpAddress = endAddressArray.join('.');
		addressRange1 = startIpAddress;
		addressRange2 = endIpAddress;
		if(addressBlockMandatoryBits >=31 || userMaskBits >= 31){
			resultArray[j][3] = '--';
			resultArray[j][4] = broadcastAddr.join('.');
		}else{
			resultArray[j][2] = addressRange1;
			resultArray[j][3] = addressRange2;
			resultArray[j][4] = broadcastAddr.join('.');
		}
		
		//console.log(j+" ============ "+networkAddress+" ============ "+addressRange1+"======"+addressRange2+"====="+" =========== "+broadcastAddr);
	}
}
...
function createTable(resultArray) {
	...
	for (c = 0; c < row_num; c++) {
		...
		for (k = 0; k < cell_num; k++) {
			...
			if(c == 0){
				if(k==0){
					cell[k].setAttribute('style','font-weight:bold;width:90px;line-height:50px;');
				}else{
					cell[k].setAttribute('style','font-weight:bold;width:273px;line-height:50px;');
				}
				switch (k){
					case 0:
						cont = document.createTextNode('#'); //No I18N
						break;
					case 1:
						cont = document.createTextNode('Dirección IP'); //No I18N
						break;
					case 2:
						cont = document.createTextNode('Primera Valida'); //No I18N
						break;
					case 3:
						cont = document.createTextNode('Ultima Valida'); //No I18N
					break;
					case 4:
						cont = document.createTextNode('Broadcast'); //No I18N
						break;
				}
			}else{
				if(k==0){
					cell[k].setAttribute('style','width:90px;line-height:50px;');
				}else{
					cell[k].setAttribute('style','width:273px;line-height:50px;');
				}
				switch (k){
					case 0:
